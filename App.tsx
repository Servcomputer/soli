...
const App: React.FC = () => {
  ...
  // Handle resize for layout calculations
  useEffect(() => {
  ...
  const findDropTarget = (x: number, y: number): Selection => {
    ...
    for (let i = 0; i < tableauRefs.current.length; i++) {
      ...
      if (ref) {
        const rect = ref.getBoundingClientRect();
        if (x >= rect.left - 5 && x <= rect.right + 5 && y >= rect.top && y <= rect.bottom + 400) return { source: 'tableau', pileIndex: i };
      }
    }
    return null;
  };
  ...
  const verticalOffset = isMobile ? 12 : 35;

  return (
    <div className="h-screen w-screen flex flex-col items-center p-2 md:p-4 overflow-hidden">
      {/* HUD */}
      <div className={`w-full max-w-6xl flex items-center justify-between mb-2 md:mb-6 border p-2 md:p-4 rounded-xl md:rounded-2xl backdrop-blur-xl z-[50] transition-colors ${hudClasses}`}>
        {/* Left Section: Stats */}
        <div className="flex-1 flex gap-2 md:gap-6 justify-start overflow-hidden">
          <div className="flex flex-col">
            <span className={`text-[7px] md:text-xs uppercase tracking-widest ${hudLabelClasses}`}>Moves</span>
            <span className={`text-[11px] md:text-xl ${hudValueClasses}`}>{gameState.movesCount}</span>
          </div>
          <div className="flex flex-col">
            <span className={`text-[7px] md:text-xs uppercase tracking-widest ${isNeonMode ? 'text-pink-500 font-neon neon-text-pink' : 'text-zinc-500 font-bold'}`}>Time</span>
            <span className={`text-[11px] md:text-xl ${hudValueClasses}`}>{formatTime(currentTime)}</span>
          </div>
        </div>

        {/* Center Section: Title */}
        <div className="flex-none px-1 md:px-4 text-center">
          <h1 className={`text-[10px] sm:text-lg md:text-2xl font-black italic tracking-tighter uppercase leading-none ${isNeonMode ? 'font-neon text-white neon-text-cyan' : 'text-zinc-800'}`}>
            RADIANT<br className="sm:hidden" /> SOLITAIRE
          </h1>
        </div>

        {/* Right Section: Buttons */}
        <div className="flex-1 flex gap-1 md:gap-3 justify-end items-center">
          <button onClick={() => setIsNeonMode(!isNeonMode)} className={`p-1 md:p-2 rounded-lg border transition-all ${isNeonMode ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-zinc-100 border-zinc-300 text-zinc-700 hover:bg-zinc-200'}`}>
            {isNeonMode ? <svg className="w-3.5 h-3.5 md:w-5 md:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z" /></svg> : <svg className="w-3.5 h-3.5 md:w-5 md:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>}
          </button>
          <button onClick={() => setShowThemes(true)} className={`p-1 md:p-2 rounded-lg border transition-all ${isNeonMode ? 'bg-white/5 border-white/10 text-white hover:bg-white/10' : 'bg-zinc-100 border-zinc-300 text-zinc-700 hover:bg-zinc-200'}`}>
          ...
          <button onClick={handleNewGame} className={`px-2 md:px-3 py-1 md:py-1.5 text-[8px] md:text-xs font-black rounded-lg uppercase transition-all ${isNeonMode ? 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-[0_0_10px_rgba(8,145,178,0.3)] font-neon' : 'bg-zinc-800 hover:bg-zinc-700 text-white'}`}>Reset</button>
        </div>
      </div>

      {/* Game Board Container */}
      <div className="w-full max-w-6xl flex flex-col gap-2 md:gap-4 flex-1 overflow-y-auto px-1 no-scrollbar">
        {/* Top Row: Stock, Waste and Foundation */}
        <div className="grid grid-cols-7 gap-1 md:gap-3 h-fit flex-shrink-0">
          <div className="col-span-2 flex gap-1 md:gap-3">
            <div className="w-1/2" onClick={handleStockClick}>
               {gameState.stock.length > 0 ? (
                 <Card card={{ id: 'stock', suit: Suit.CLUBS, rank: Rank.ACE, isFaceUp: false }} isNeon={isNeonMode} backTheme={backTheme} speed={gameSpeed} />
               ) : (
                 <div className={`w-full aspect-[5/7] rounded-lg border-2 border-dashed flex items-center justify-center cursor-pointer transition-colors ${isNeonMode ? 'border-white/10 hover:bg-white/5 text-white/20' : 'border-black/10 hover:bg-black/5 text-black/20'}`}>
                    <span className={`text-xl md:text-3xl ${isNeonMode ? 'font-neon' : ''}`}>↺</span>
                 </div>
               )}
            </div>
          ...
          <div className="col-span-4 flex justify-between gap-1 md:gap-2">
            {(Object.values(Suit)).map(suit => {
              ...
              const slotBg = isNeonMode ? 'bg-black/40 border-[1px] md:border-2' : 'bg-black/10 border-[1px] md:border-2 border-white/10';
              const slotBorder = isNeonMode ? (isRed ? 'border-pink-500/20' : 'border-cyan-500/20') : 'border-white/10';

              return (
                <div key={suit} ref={el => { foundationRefs.current[suit as Suit] = el; }} className={`relative w-full aspect-[5/7] rounded-lg flex items-center justify-center overflow-hidden transition-colors ${slotBg} ${slotBorder}`}>
                  <SuitIcon suit={suit} className={`w-4 h-4 md:w-10 md:h-10 opacity-10 ${isNeonMode ? (isRed ? 'text-pink-500' : 'text-cyan-500') : 'text-zinc-100'}`} />
        ...
        {/* Tableau */}
        <div className="grid grid-cols-7 gap-1 md:gap-3 flex-1 h-full pb-20">
          {gameState.tableau.map((pile, pileIdx) => (
            <div key={pileIdx} ref={el => { tableauRefs.current[pileIdx] = el; }} className="relative flex-1 min-h-[300px]">
              {pile.length === 0 && (
                <div onClick={() => selectCard({ source: 'tableau', pileIndex: pileIdx })} className={`w-full aspect-[5/7] rounded-lg border-2 border-dashed transition-colors ${isNeonMode ? 'border-white/5 bg-white/5 hover:bg-white/10' : 'border-black/5 bg-black/5 hover:bg-black/10'}`}></div>
              )}
      ...
      {/* Drag Preview */}
      {dragState && (
        <div className="fixed pointer-events-none z-[9999]" style={{ left: `${dragState.currentPos.x - dragState.offset.x}px`, top: `${dragState.currentPos.y - dragState.offset.y}px`, width: `${isMobile ? windowWidth / 7.5 : windowWidth / 10}px` }}>
          {dragState.cards.map((card, i) => (
            <div key={card.id} className="absolute w-full" style={{ top: `${i * verticalOffset}px` }}>
      ...
      {showThemes && (
        <div className="fixed inset-0 z-[115] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className={`p-4 md:p-8 rounded-2xl shadow-2xl max-w-sm w-full relative max-h-[90vh] overflow-y-auto ${isNeonMode ? 'bg-[#0a0a0f] border border-pink-500/30 text-white' : 'bg-white text-zinc-900'}`}>
            <button onClick={() => setShowThemes(false)} className={`absolute top-4 right-4 transition-colors ${isNeonMode ? 'text-pink-500 hover:text-white' : 'text-zinc-400 hover:text-zinc-900'}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            <h2 className={`text-xl font-black mb-4 tracking-widest uppercase ${isNeonMode ? 'font-neon neon-text-pink' : 'font-bold'}`}>Temas</h2>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[
                { id: 'blue', color: 'bg-[#001a33] border-[#00f2ff]', name: 'Sapphire' },
                { id: 'red', color: 'bg-[#2a0015] border-[#ff007f]', name: 'Ruby' },
                { id: 'green', color: 'bg-[#002211] border-[#39ff14]', name: 'Emerald' },
                { id: 'black', color: 'bg-[#0f0f15] border-[#7f00ff]', name: 'Onyx' }
              ].map((theme) => (
                <div key={theme.id} onClick={() => setBackTheme(theme.id as CardBackTheme)} className={`cursor-pointer flex flex-col items-center gap-2 p-2 rounded-xl border transition-all ${backTheme === theme.id ? 'bg-white/10 ring-2 ring-pink-500 border-transparent' : 'border-white/5 hover:bg-white/5'}`}>
                  <div className={`w-full aspect-[5/7] rounded-lg border flex items-center justify-center ${theme.color}`}>
                    <SuitIcon suit={Suit.SPADES} className="w-6 h-6 text-white opacity-20" />
                  </div>
                  <span className="text-[10px] font-black uppercase">{theme.name}</span>
                </div>
              ))}
            </div>
            <button onClick={() => setShowThemes(false)} className={`w-full py-3 text-sm font-black rounded-xl uppercase transition-colors ${isNeonMode ? 'bg-pink-600 hover:bg-pink-500 text-white font-neon' : 'bg-zinc-800 hover:bg-zinc-700 text-white'}`}>Fechar</button>
          </div>
        </div>
      )}
    </div>
